...
re.MULTILINE)
# Znajdź wszystkie wystąpienia action i action_input:
actions = action_pattern.findall(text)
action_inputs = action_input_pattern.findall(text)
# Znajdź ostatnie wystąpienie action i action_input:
last_action = actions[-1] if actions else None
last_action_input = action_inputs[-1] if action_inputs else None
print("Ostatnia akcja:", last_action)
print("Ostatnie informacje wejściowe:", last_action_input) 
# Ostatnia akcja: search_on_google
# Ostatnie informacje wejściowe: jakieś inne zapytanie
