...
# Funkcja wykonuje wyszukiwanie wektorowe, a następnie zadaje modelowi GPT-3.5-turbo pytanie
def search_and_chat(user_query, k=1):
  # Wykonaj wyszukiwanie wektorowe
  search_results = vector_search(user_query, k) 
  print(f"Wyniki wyszukiwania: {search_results}\n\n")
  prompt_with_context = f"""Kontekst:{search_results}\ Odpowiedz na pytanie: {user_query}"""
  # Utwórz listę komunikatów dla czata
  messages = [
    {"role": "system", "content": """Odpowiedz proszę na pytanie przedstawione przez użytkownika. Odpowiadając korzystaj jedynie z dostarczonego kontekstu. Jeśli nie znasz odpowiedzi, odpowiedz \"Nie wiem.\" """},
    {"role": "user", "content": prompt_with_context},
  ]
  # Pobierz odpowiedź modelu
  response = client.chat.completions.create(
    model="gpt-3.5-turbo", messages=messages)
  # Wyświetl odpowiedź asystenta
  print(f"""Odpowiedź:
  {response.choices[0].message.content}""")
# Przykładowe wyszukiwanie i czat
search_and_chat("Jaka jest misja firmy Unicorn Enterprises?")
